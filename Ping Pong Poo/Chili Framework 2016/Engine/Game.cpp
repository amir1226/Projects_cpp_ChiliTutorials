	...
	gfx( wnd )
{
	...
	std::mt19937 rng(rd());
	std::uniform_int_distribution<int> xDist(0, 780);
	std::uniform_int_distribution<int> yDist(0, 580);
...
void Game::UpdateModel()
{
	int vx = 10;
	thePoo.pooMove();
	thePoo.barTouch(bar0.x, bar0.y, Bar::width, Bar::height);
	thePoo.barTouch(bar1.x, bar1.y, Bar::width, Bar::height);
...
void Game::makeBar(int x, int y, int width, int height)
{
	for (int i = 0; i < height ; i++)
	{
		for (int j = 0; j < width; j++)
		{
			gfx.PutPixel(x + j, y + i, 255, 255, 255);
		}
	}
}

void Game::makeGameOver(int x, int y)
{
...
{
	if (thePoo.finishGame)
	{
		makeGameOver(400 - 84, 300 - 64);
	}
	else
	{
	makePoo(thePoo.x, thePoo.y);
	makeBar(bar0.x, bar0.y, Bar::width, Bar::height);
	makeBar(bar1.x, bar1.y, Bar::width, Bar::height);
	}
}
